...
export const playground = () => {
    ...
    return (`<div>
        ...
        <span>
            <ks-loading class="${cssClass}" message="${message}" show-message="${showMessage}" type="${type}" />
        </span>
    </div>`);
}
...
export const loadingOverlay = () => {
    ...
    return (`<div style="width:300px; position:relative;">
        ...
        <ks-loading-overlay size="${size}" absolute="${absolute}" type="${type}" message="${message}" show-message="${showMessage}" />
    </div>`);
}